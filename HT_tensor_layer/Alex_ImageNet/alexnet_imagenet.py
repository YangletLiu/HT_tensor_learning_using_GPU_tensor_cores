...
parser.add_argument('--epochs', default=7, type=int, metavar='N',  # 50   epoch
                    help='number of total epochs to run')
parser.add_argument('--start-epoch', default=0, type=int, metavar='N',
                    help='manual epoch number (useful on restarts)')
# parser.add_argument('-b', '--batch-size', default=256, type=int, #batch_size
    ...
    with torch.no_grad():
        for i, (inputs, target) in enumerate(train_loader):
            ...
            for param in model.parameters():
                grad_sum.append(torch.zeros_like(param, device=device))
            for j in range(10):
                loss_list = []
                noise_list = []
                ...
                for k in range(len(grad_sum)):
                    grad_sum[k] = grad_sum[k] + grad[k]
                # print('current :', j )
            grad_sum = [g / 10 for g in grad_sum]
            set_es_grad(model, grad_sum)
            optimizer.step()
...
def es_train(model, train_loader, val_loader):
    if_mirror = True
    num_directions = 100  # natural gradient iteration number
    elite_rate = 0.4
