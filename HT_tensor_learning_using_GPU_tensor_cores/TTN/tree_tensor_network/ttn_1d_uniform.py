...
def _ascend_op_2site_to_1site_partial(mpo_2site, iso_021):
  ...
  M = len(op2L)  # MPO bond dimension   这里就是 1/ 后面的调用，这里会为2

  #print("zai zhe ting dun！： {}",M)
  terms = []
  for m in range(M):
...
def opt_energy_env_2site(isos_012, h_mpo_2site, states_1site_above):
  ...
  def _ascend_op_2site_to_2site_many(mpo_2site, isos):
    ...
    for l in range(len(isos)):
      mpo_2site = ascend_op_2site_to_2site(mpo_2site, *isos[l]) #8*8,16*16,16*16,16*16,1*1     
  ...
  isos_wt = isos_with_transposes(isos_012) #将isos_012 和 isos_021 组合起来
  iso_012, iso_021 = isos_wt[0]
  isos_wt_above = isos_wt[1:]
  levels_above = len(isos_wt_above) #第一次执行时(l=bottom)，这里为4
  ...
  def _compute_env(lvl, reflect=False):
    ...
    for lvl2 in reversed(range(lvl)):
      ...
      if reflect:
        envR = _descend_energy_env_L(envR, iso_021_l2) # len(envL) 和 len(envL) 一直是1       
    ...
    envL = sum(
        backend.ncon([eL, ihR], [(1, -1), (1, -2, -3)])  #len(envL) 一直都是1，目前不需要 for
  ...
  weightsum = 0.0
  env_total = []
  for lvl in range(levels_above): # 0~3 
...
def opt_energy_layer(isos_012,#iso_012
                     h_op_1site, #h1
                     h_mpo_2site, #h2
                     states_1site_above, #states
                     itr, # 1
                     graphed=False, #true
                     graph_level=None,
                     decomp_mode="eigh", # svd_full
                     decomp_device=None, #none
                     envsq_dtype=None,
                     timing=False):
...
def opt_tree_energy(isos_012,
  ...
  for l in range(L):
    ...
    if shp[0] == shp[1] * shp[2]:
      if graphed:
        H = backend.ascend_uniform_op_local_graph(  #这个和下面的不带 grapg 的函数一模一样，没区别
  ...
  for j in range(10):
    ...
    states = [None] * bottom + states # num_layers + 1
