...
{		
	...
	/*dt *d_sumXXT;
	cudaMalloc((void**)&d_sumXXT,sizeof(dt)*a*a*3);


	cublasScopy(handle,a*a,d_X1_X1 ,1,d_sumXXT,1);
	cublasScopy(handle,b*b,d_X2_X2 ,1,d_sumXXT+a*a,1);
	cublasScopy(handle,c*c,d_X3_X3 ,1,d_sumXXT+a*a+b*b,1);
...
cudaMalloc ((void**)&d_W , sizeof(dt) * a * 3);
cudaMalloc ((void**)&d_info, sizeof(int ) * 3);
 cusolverDnSsyevjBatched_bufferSize(
  ...
  cusolverDnSsyevjBatched(
	...
	cublasScopy(handle,b*k[4],d_sumXXT+a*a+a*(a-k[4]) ,1,d_Ux5,1);
	cublasScopy(handle,c*k[2],d_sumXXT+2*a*a+a*(a-k[2]) ,1,d_Ux3,1);*/
	...
	cudaDeviceSynchronize();*/
	dt *d_U,*d_Ux2_t;
	cudaMalloc((void**)&d_U,sizeof(dt)*a*b*k[1]);
